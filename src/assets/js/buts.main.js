$(function () {
  ...
  $itemPartyNavi = $itemSlideParty.find('.slide-navi'),
  ...
  function itemPartInit(){
    if(!isMobile){
      $itemPartyList.on('mouseenter', function(){
      ...
      $itemPartyList.on('swipeleft', function(event){
        itemPartyMove(1);
  ...
  function itemPartyNavi(){
    $itemPartyNavi.show().on('click', function(e){
      e.preventDefault();
  ...
  function itemPartyMove(direction){
    if(direction > 0){
      ...
      $itemPartyList.stop().prepend($('.item-slide.party .item-row').last()).prepend($('.item-slide.party .item-row').last()).css({left: -itemPartyRowWidth + 'px'});
  ...
  $eventNavi = $eventSlide.find('.slide-navi'),
  $eventIndicator = $eventSlide.find('.indicator'),
  $eventButton = null;
  ...
  function eventInit(){
    if(!isMobile){
      ...
      $eventList.on('swipeleft', function(event){
        eventMove(1);
      }).on('swiperight', function(event){
        eventMove(-1);
      });
  ...
  function eventNavi(){
    $eventNavi.show().on('click', function(e){
      e.preventDefault();
  ...
  function eventMove(direction){
    if(direction > 0){
      ...
      $eventList.stop().prepend($('.event-slide .event-row').last()).css({left: -eventRowWidth + 'px'});
  ...
  $itemGiftNavi = $itemSlideGift.find('.slide-navi'),
  $itemGiftIndicator = $itemSlideGift.find('.indicator'),
  $itemGiftButton = null;
  ...
  function itemGiftInit(){
    if(!isMobile){
      $itemGiftList.on('mouseenter', function(){
      ...
      $itemGiftList.on('swipeleft', function(event){
        itemGiftMove(1);
      }).on('swiperight', function(event){
        itemGiftMove(-1);
      });
  ...
  function itemGiftNavi(){
    $itemGiftNavi.show().on('click', function(e){
      e.preventDefault();
  ...
  function itemGiftMove(direction){
    if(direction > 0){
      ...
      $itemGiftList.stop().prepend($('.item-slide.gift .item-row').last()).css({left: -itemGiftRowWidth + 'px'});
